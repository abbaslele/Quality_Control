...
function toFarsiNumber(num) {
    let formattedNumber = Number(num).toLocaleString(Qt.locale("fa_IR"), 'f', 0).replace(Qt.locale("fa_IR").groupSeparator, "");
    if (num < 10) {
        formattedNumber = "۰" + formattedNumber;
    }
...
function formatCurrentTime() {
    let date = new Date();
    let hours = Number(date.getHours()).toLocaleString(Qt.locale("fa_IR"), 'f', 0).replace(Qt.locale("fa_IR").groupSeparator, "");
    let minutes = Number(date.getMinutes()).toLocaleString(Qt.locale("fa_IR"), 'f', 0).replace(Qt.locale("fa_IR").groupSeparator, "");
...
function jalali_to_gregorian(jy, jm, jd) {
    ...
    var gm; // Fixed: Added missing semicolon
